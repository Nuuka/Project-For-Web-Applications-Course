<html>
    ...
    <div id="center">
        ...
        <!--Form with option to post the story and the users 2 choices (overide width to default to center the textarea)-->
         <form action="EditNodeServlet" method="post"  id="postStoryForm"> 
        	<input id="update" type="hidden" name="update" value="update">
        	<input id="nodeid" type="hidden" name="nodeid" value="<%=request.getAttribute("node")%>">
			...
			<script>
				...
				if('<%=session.getAttribute("pictureString2")%>' == "null"){
					...
					image.src = 'data:image/png;base64,'+ '<%=request.getAttribute("pictureText")%>';
				}
			</script>
			<br/>
            <p style="text-align:left;padding:8px;margin:0px;">Story:</p>
            <!--Main text area for the users Story-->
            <textarea name="postText" id="msg" style="resize:both" rows="8" cols="72"><%=request.getAttribute("text")%></textarea><br/>
            <!--Table to hold the 2 chocie text boxes-->
            <table style="margin-left: auto;margin-right:auto">
                <tr>
                    <td style="color: #ffffff">
                        <label  style="color:#ffffff" for="msg">Choice 1:</label><br/>
                        <textarea name="choice1" id="msg" style="resize:both"rows="4" cols="35"><%=request.getAttribute("choice1text")%></textarea>
                        <!--temp html-->
                        
                    </td>
                    <td style="color: #ffffff">
                        <label style="color:#ffffff" for="msg">Choice 2:</label><br/>
                        <textarea name="choice2" id="msg" style="resize:both"rows="4" cols="34"><%=request.getAttribute("choice2text")%></textarea>
                        <!--temp html-->
